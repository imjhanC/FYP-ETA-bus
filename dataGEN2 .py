...
def generate_bus_dataset(n_rows=100000):
    ...
    # Fixed distances between consecutive stops (in km)
    distances = {
        1: 1.44,  # UTAR → KJ370
        2: 0.43,  # KJ370 → SMK SG Long
        3: 0.81,  # SMK SG Long → Green Acre
        4: 0.72,  # Green Acre → Club House
        5: 0.84   # Club House → UTAR (completing the loop)
    }
    ...
    while len(data['timestamp']) < n_rows:
        ...
        if 6 <= hour <= 22:
            ...
            # Modified passenger count generation to ensure it's always > 0
            passenger_count = int(np.random.normal(
                base_passengers * weather_passenger_factor,
                base_passengers * 0.2
            ))
            passenger_count = max(1, min(50, passenger_count))  # Changed from max(0,...) to max(1,...)
            ...
            # Update sequence and bus (strictly following 1→2→3→4→5→1)
            current_sequence = next_sequence
            if current_sequence == 1:  # Only switch bus when completing full loop
...
df.to_csv('bus_eta_dataset2.csv', index=False)